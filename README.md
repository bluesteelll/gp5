...
Бизнес-цель - рекомендовать пользователю самые релевантные заведения. Но разведочный анализ вскрыл
проблему: рейтинг «грязный» - у многих отзывов тон текста расходится с собственной оценкой. Значит, прежде чем рекомендовать, отзывы стоит чистить - отсюда и вторая
задача.

- **Задача 1 - рекомендации.** Полносвязная сеть по табличным признакам пользователя и заведения
  предсказывает, на сколько звёзд человек оценит место, и собирает подборку «специально для вас».
- **Задача 2 - анализ отзывов.** Модель читает текст: определяет тональность, выделяет аспекты (еда,
  сервис, цена). Ищем, когда тональность расходится с оценкой. Мы придумали два применения модели, которые позволяет сделать рейтинг более корректным: при публикации отзыва с «нестыкующейся» оценкой - мягкая UX-подсказка («написано скорее негативно, а стоит 5★ -
...
- **быстрые итерации** - обучение и EDA занимают минуты, а не часы, в том числе на ноутбуке (CPU/MPS);
...
Архитектуру удобно читать на схеме ниже (нарисована через [mermaid](https://mermaid.js.org/syntax/flowchart.html)).
...
что они не согласованы. По одной звезде задача неразрешима (`stars-only` ≈ 0.50 AUC - классы специально выровнены), поэтому модель опирается на текст и его соответствие оценке.

```mermaid
flowchart TD
    TXT["📝 Текст отзыва · токены"]
    ST["⭐ Оценка 1–5"]

    TXT --> EMB["Embedding<br/>словарь → 128"]
    EMB --> CONV["3 × Conv1d<br/>ядра 3·4·5, по 100 фильтров + ReLU"]
    CONV --> POOL["max-over-time pooling<br/>→ вектор текста · 300"]

    ST --> SEMB["Embedding 5 → 16<br/>вектор звезды"]

    POOL --> CAT["concat · 316"]
    SEMB --> CAT
    CAT --> DROP["Dropout 0.5"]
    DROP --> FC["Linear → 2 логита"]
    FC --> P["softmax → P(mismatch)"]
    P --> OUT(["🚩 P > порог →<br/>подсказка автору · понижение веса оценки"])
...
> Текстовую ветку кодирует TextCNN (свёртки по n-граммам 3/4/5 + max-pooling), звезду - маленький
> эмбеддинг; векторы склеиваются, и классификатор выдаёт `P(mismatch)`. Высокое значение → подсказка
> автору при публикации и понижение веса «шумной» оценки для рекомендательной системы.
